...
func _on_server_connected() -> void:
    ...
    has_sent_join = true
    game_server_client.join_match(config.match_id, config.client_id)
...
func _apply_event_to_presentation(message: Dictionary) -> void:
    ...
    if str(event_dictionary.get("type", "")) == "dice_rolled":
        ...
        board_camera_controller.focus_on_space(int(event_dictionary.get("to_position", 0)), false)
    ...
    if message_type == "match_event":
        ...
        if event is Dictionary:
            print("Evanopolis client received event: %s@%d" % [
                str((event as Dictionary).get("type", "")),
